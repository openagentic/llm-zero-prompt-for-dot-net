...
public class CreateAzurePowerShellScript(ILLMConnectorV2 llmConnector)
{
    ...
    public CreateAzurePowerShellScript Inp(string question, string context)
    {
        _question = question;
        _context = context;
        return this;
    ...
    public async Task<OperationResult<string, string, string>> ExAsAzurePowerShellScriptAsync(int? numberOfWords = null)
    {
        var target = GetType().FullName;
        var tdca = new LLMTaskDescriptionBuilder<string, string>(target, LLMTaskDescriptionInputFormats.PowerShellScriptRequirements, LLMTaskDescriptionOutputFormats.AzurePowerShellScript, _llmConnector);
