...
public class CreateMicrosoftBICEPTemplate(ILLMConnectorV2 llmConnector)
{
    ...
    public CreateMicrosoftBICEPTemplate Inp(string question, string context)
    {
        _question = question;
        _context = context;
        return this;
    ...
    public async Task<OperationResult<string, string, string>> ExAsBICEPAsync(int? numberOfWords = null)
    {
        var target = GetType().FullName;
        var tdca = new LLMTaskDescriptionBuilder<string, string>(target, LLMTaskDescriptionInputFormats.Paragraph, LLMTaskDescriptionOutputFormats.AzureBICEPTemplate, _llmConnector);
