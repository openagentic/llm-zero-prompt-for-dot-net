...
public class TextSimplification(ILLMConnectorV2 llmConnector)
{
    ...
    public TextSimplification Inp(string question, string context)
    {
        _question = question;
        _context = context;
        return this;
    ...
    public async Task<OperationResult<string, string, string>> ExAsSentenceAsync()
    {
        var target = GetType().FullName;
        var tdca = new LLMTaskDescriptionBuilder<string, string>(target, LLMTaskDescriptionInputFormats.Paragraph, LLMTaskDescriptionOutputFormats.Paragraph, _llmConnector);
